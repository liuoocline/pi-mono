...
export function convertMessages(
  ...
  // Post-conversion role ordering repair: merge consecutive same-role messages.
  // transformMessages() and the conversion loop above can skip errored/empty
  // assistant messages, leaving consecutive user or assistant messages that
  // strict APIs (e.g., NVIDIA Qwen 3.5) reject with "roles must alternate".
  return repairRoleOrdering(params);
}

/**
 * Merge consecutive messages with the same role to ensure strict role alternation.
 * - Consecutive user messages: merge content arrays/strings.
 * - Consecutive assistant messages: merge content arrays/strings and tool_calls.
 * - System/tool messages are left untouched.
 */
function repairRoleOrdering(params: ChatCompletionMessageParam[]): ChatCompletionMessageParam[] {
  if (params.length <= 1) return params;

  const result: ChatCompletionMessageParam[] = [params[0]];

  for (let i = 1; i < params.length; i++) {
    const prev = result[result.length - 1];
    const curr = params[i];

    // Only merge consecutive user or assistant messages
    if (prev.role === curr.role && (curr.role === "user" || curr.role === "assistant")) {
      if (curr.role === "user") {
        // Merge user messages by combining content
        const prevContent =
          typeof prev.content === "string"
            ? [{ type: "text" as const, text: prev.content }]
            : ((prev.content as ChatCompletionContentPart[]) ?? []);
        const currContent =
          typeof curr.content === "string"
            ? [{ type: "text" as const, text: curr.content }]
            : ((curr.content as ChatCompletionContentPart[]) ?? []);
        result[result.length - 1] = {
          role: "user",
          content: [...prevContent, ...currContent],
        };
      } else {
        // Merge assistant messages by combining content and tool_calls
        const prevAssistant = prev as ChatCompletionAssistantMessageParam;
        const currAssistant = curr as ChatCompletionAssistantMessageParam;

        const prevContent =
          prevAssistant.content == null
            ? []
            : typeof prevAssistant.content === "string"
              ? [{ type: "text" as const, text: prevAssistant.content }]
              : prevAssistant.content;
        const currContent =
          currAssistant.content == null
            ? []
            : typeof currAssistant.content === "string"
              ? [{ type: "text" as const, text: currAssistant.content }]
              : currAssistant.content;

        const merged: ChatCompletionAssistantMessageParam = {
          role: "assistant",
          content: [...prevContent, ...currContent],
        };
        const prevCalls = prevAssistant.tool_calls ?? [];
        const currCalls = currAssistant.tool_calls ?? [];
        if (prevCalls.length > 0 || currCalls.length > 0) {
          merged.tool_calls = [...prevCalls, ...currCalls];
        }
...
function detectCompat(model: Model<"openai-completions">): Required<OpenAICompletionsCompat> {
  ...
  const isQwenModel = model.id.toLowerCase().includes("qwen");
  ...
  return {
    ...
    thinkingFormat: isZai ? "zai" : isQwenModel ? "qwen" : "openai",
    openRouterRouting: {},
