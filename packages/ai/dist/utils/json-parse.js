    ...
    try {
        ...
        try {
            ...
            if (errorMsg.includes("JSON") || errorMsg.includes("position")) {
                console.warn("[parseStreamingJson] JSON parse failed after fix attempt", `\nError: ${errorMsg}`, `\nJSON (first 200 chars): ${partialJson.slice(0, 200)}`);
            }
...
export function safeStringify(value, fallback = "{}") {
    try {
        ...
        console.warn("[safeStringify] JSON serialization failed", `\nError: ${errorMsg}`, `\nValue type: ${typeof value}`, `\nValue (first 200 chars): ${String(value).slice(0, 200)}`);
        try {
